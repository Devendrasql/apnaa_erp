...
const createPurchaseOrder = async (req, res, next) => {
  ...
  try {
    ...
    // Enforce branch scope: active branch must match body branch
    const activeBranchId = Number(req.branchId);
    if (!Number.isFinite(activeBranchId)) {
      return res.status(400).json({ success: false, message: 'branchId required (query or x-branch-id)' });
    }
    if (Number(bodyBranchId) !== activeBranchId) {
      return res.status(403).json({
        success: false,
        message: `Cross-branch write not allowed. Use branch_id=${activeBranchId}.`
      });
    }
    ...
    for (const it of items) {
      ...
      const taxP  = toNumber(it.tax_percentage, 12); // default 12% (adjust to your business logic)
...
const getAllPurchaseOrders = async (req, res, next) => {
  try {
    ...
    // Default to active branch if provided (prevents leakage)
    const effectiveBranchId = branch_id ?? (Number.isFinite(activeBranchId) ? activeBranchId : undefined);
    if (effectiveBranchId != null) { whereClauses.push('po.branch_id = ?'); whereParams.push(effectiveBranchId); }
...
const getPurchaseOrderById = async (req, res, next) => {
  try {
    ...
    // Restrict cross-branch read if branch context is provided
    if (Number.isFinite(activeBranchId) && purchaseOrder.branch_id !== activeBranchId) {
      return res.status(403).json({ success: false, message: 'Forbidden for this branch.' });
    }
...
const receivePurchaseOrder = async (req, res, next) => {
  ...
  // Accept either `items` (route validator) or legacy `poItems`
  const items = Array.isArray(req.body.items) ? req.body.items : req.body.poItems;
  ...
  try {
    ...
    // Enforce branch scope: active branch must match PO branch
    const activeBranchId = Number(req.branchId);
    if (!Number.isFinite(activeBranchId)) {
      return res.status(400).json({ success: false, message: 'branchId required (query or x-branch-id)' });
    }
    if (poDefaults.branch_id !== activeBranchId) {
      return res.status(403).json({ success: false, message: 'Forbidden for this branch.' });
    }
