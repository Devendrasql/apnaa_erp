...
const SupplierFormModal = ({ open, onClose, onSubmit, supplier, isLoading }) => {
    ...
    if (open) {
      if (supplier) {
        Object.keys(supplier).forEach((key) => {
          if (key === 'is_active') setValue(key, !!supplier[key]);
          else setValue(key, supplier[key] || '');
        });
      } else {
        reset();
      }
  ...
  const { mutate: fetchGstDetails, isLoading: isFetchingGst } = useMutation(api.getSupplierByGST, {
    onSuccess: (response) => {
  ...
  const handleFetchGst = () => {
    if (gstNumberValue) fetchGstDetails(gstNumberValue);
    else toast.error('Please enter a GST number first.');
  };
