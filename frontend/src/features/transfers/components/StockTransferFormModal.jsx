// In frontend/src/features/transfers/components/StockTransferFormModal.jsx
...
const StockTransferFormModal = ({ open, onClose, onSubmit, isLoading }) => {
  ...
  const { data: stockData, isLoading: isLoadingStock } = useQuery(
    ...
    { enabled: !!fromBranchId, select: (res) => res.data.data }
  );
  ...
  useEffect(() => {
    if (open && currentBranch?.id) setValue('from_branch_id', currentBranch.id);
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <TextField
                    ...
                    disabled={isLoadingBranches || !isElevated}
                  >
                    {branches.map((branch) => (
                ...
                <Controller
                  ...
                  render={({ field: controllerField, fieldState }) => (
                    <Autocomplete
                      ...
                      renderInput={(params) => (
                        <TextField {...params} label="Select Product Batch" required error={!!fieldState.error} helperText={fieldState.error?.message} />
                      )}
                    />
                  )}
                />
