...
export default function AbacPoliciesPage() {
  ...
  React.useEffect(() => {
    ...
    // each policy: { id?, name, effect, action, subject, condition: string (JSONLogic/CEL) }
    setRows(Array.isArray(src) ? src : []);
  }, [policiesQ.data]);

  const addRow = () => setRows(prev => [...prev, { name: '', effect: 'allow', action: '', subject: '', condition: '' }]);
  ...
  const save = () => {
    ...
    const bad = rows.find(r => !r.name || !r.action || !r.subject);
              ...
              <TableBody>
                {rows.map((r, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell><TextField size="small" fullWidth value={r.subject} onChange={e => updateRow(idx, 'subject', e.target.value)} placeholder="e.g. sales" /></TableCell>
                    <TableCell>
                      <TextField
                        size="small"
                        fullWidth
